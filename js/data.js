...
const DATA = {
  ...
  "fish": [
    ...
    },
    {
      ...
      "bait": "루어 (동굴 근거리 캐스팅)",
      "tips": "그로브 동굴 전용. 좁은 근거리 수역이라 루어 컨트롤이 까다로운 편."
    },
    {
      ...
      "tips": "그로브 수로·댐 강변 양쪽에서 포획 사례 확인. 근거리 수로 파이팅이 어려운 편. 4000D 헤비 드래그 릴 해금용 트레이드 대상 3종 중 하나."
    },
    {
      "id": "baby-shark",
      "image": null,
      ...
      "tips": "40레벨 승급 제출 대상(3번째). 소장급 중 출현지가 가장 넓어 다른 레드를 노릴 때 자주 대신 걸림. 줄을 40m 이상 끌고 나가는 강한 파이팅."
    },
    {
      "id": "atlantic-salmon",
      ...
      "tips": "최고 난도로 꼽히는 레드. 노리다 보면 새끼상어·타폰이 대신 걸리는 일이 잦음. 깊고 먼 포인트 위주. 4000D 헤비 드래그 릴 해금용 트레이드 대상 3종 중 하나."
    },
    {
      "id": "tigerfish",
      ...
      "tips": "그로브 베이 유일 출현(조 리드 집 근처, 맵 좌상단). 줄을 멀리 끌지 않는 대신 순간 장력이 급상승하는 타입 — 장력 레드존 주의. 4000D 헤비 드래그 릴 해금용 트레이드 대상 3종 중 하나."
    },
    {
      "id": "bigeye-tarpon",
      "image": null,
  ...
  "missions": {
    "_note": "낚시 레벨 상한 50. 10레벨 구간마다 경험치가 캡에 걸리며, 조 리드에게 승급 평가를 통과해야 다음 구간 진행. 경험치는 어종 등급 기준(회색 40 / 그린 45 / 블루 50, 근사치) — 잡는 즉시 획득되므로 탈출·인벤토리 공간 불필요. 레벨링은 찌낚시 연타가 가장 빠르고, 루어 낚시는 30레벨 이후 권장. 제출 어종·순서는 만렙 유저 공략 영상 기준.",
    "unlock": {
      "requirement": "계정(오퍼레이션) 레벨 17 달성 후(커뮤니티 제보, 미검증), 롱보우(이지)에서 낚시꾼 NPC 조 리드와 대화 → 스크롤을 올려 '수락' 선택 → 매치 종료 후(탈출 여부 무관) 우편으로 첫 낚싯대·미끼 지급",
      "note": "낚시는 별도 모드가 아니라 오퍼레이션 매치 안에서 진행. 현재 이지 난이도 3맵(롱보우·AZ3·제로댐)에서만 가능."
    },
    "certifications": [
      {
        "level": 10,
  ...
  "npc": {
    ...
    "communityAlias": "조아재",
    "map": "longbow",
    "spawnRule": "한 매치에 롱보우(이지) 낚시터 3곳 중 1곳에 랜덤 스폰. 그로브 베이(고요한 만) 집 주변이 스폰 지점 중 하나로 확인됨. 누군가 NPC를 죽이면 그 매치에서는 더 이상 등장하지 않음.",
    "spawnSpots": [
      {
        "spotId": "longbow-serenity-bay",
        "coord": null,
        "note": "그로브 베이(고요한 만) 집 주변 — 공략 영상에서 확인"
      },
      {
        "spotId": null,
        "coord": null,
    ...
    "roles": [
      "최초 낚시 해금: 대화 → 스크롤 올려 '수락' → 매치 종료 후(탈출 무관) 우편으로 첫 낚싯대·미끼 지급",
      "낚시 레벨 10/20/30/40 승급 평가 진행 (미통과 시 다음 레벨 구간 진행 불가)",
      "승급어 제출 방법: 인벤토리 건네주기가 아니라 NPC 옆 바구니에 물고기를 넣고 대화 → '물고기 제출'",
  ...
  "gear": {
    ...
    "reels": [
      ...
      },
      {
        ...
        "unlock": "레드 3종 트레이드로 해금 — 대서양 연어·타이거피시·자이언트 브라운 송어를 각 1마리씩 교환 (해외 가이드 AEON 영상 기준)",
    ...
    "tips": [
      "5클 4000D 헤비 드래그 릴은 상점 구매가 아니라 레드 3종(대서양 연어·타이거피시·자이언트 브라운 송어) 트레이드로 해금. 권장 준비물: 하이카본 루어대(30레벨) + 3000XH 릴(20,000코인) + 60lb 라인(2,700코인) + 골드/레드 미끼(30/40레벨).",
